...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  const { adminPassword, bookData } = req.body
  ...
  try {
    ...
    if (!bookData.course_id || !bookData.title || !bookData.description) {
      return res.status(400).json({ 
        error: 'Missing required fields (course_id, title, or description)' 
      })
    }
    
    // Create the book using the admin client
    ...
    console.error('Admin book creation error:')
